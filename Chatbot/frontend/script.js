document.addEventListener('DOMContentLoaded', function () {
    ...
    viewReservationBtn?.addEventListener('click', (e) => {
        e.preventDefault();
        showReservation();
    });
    ...
    function respondToMessage(message) {
        ...
        setTimeout(() => {
            if (lowerMsg.includes('cardápio') || lowerMsg.includes('menu') || lowerMsg.includes('comida')) {
                ...
                addBotMessage('Olá! Bem-vindo ao Restaurante Poliedro. Em que posso ajudar?');
            }
    ...
    function confirmReservation() {
        ...
        const reservation = { name, date, time, people };
    ...
    function showReservation() {
        const reservation = JSON.parse(localStorage.getItem('restaurantReservation'));

        if (!reservation) {
            addBotMessage('Você não tem nenhuma reserva confirmada.');
            return;
        }
        ...
        const reservationDetails = `
            ...
            <button id="cancel-res">Cancelar Reserva</button>
        `;

        addBotMessage(reservationDetails, true);

        document.getElementById('cancel-res')?.addEventListener('click', cancelReservation);
    }

    function cancelReservation() {
        localStorage.removeItem('restaurantReservation');
        addBotMessage('Sua reserva foi cancelada com sucesso.');
    }
});
